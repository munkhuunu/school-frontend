<template>
  <div class="p-6 lg:p-8 max-w-7xl">
    <div class="mb-8">
      ...
      <p class="text-sm text-stone-400 mt-1">{{ auth.user?.email }} — {{ auth.roleLabel }}</p>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
      ...
      <div>
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-sm font-semibold text-stone-500 uppercase tracking-wider">Мэдээллэгүүд</h3>
          <NuxtLink to="/announcements" class="text-xs text-emerald-600 hover:underline">Бүгд үзэх</NuxtLink>
        </div>
        <div class="space-y-2">
          <div v-if="!announcements.length" class="card p-4 text-sm text-stone-400 text-center">Мэдээллэгүү байхгүй</div>
          <div v-for="a in announcements" :key="a.announcementId" class="card p-4">
            <div class="flex items-start justify-between gap-2 mb-1">
              <p class="text-sm font-medium text-stone-800">{{ a.title }}</p>
            ...
            <p class="text-xs text-stone-400 mt-1.5">{{ a.createdAt?.slice(0, 10) }}</p>
          </div>
        </div>
      </div>
    </div>
...
const statCards = computed(() => [
  ...
  { label: 'Өнөөдөрийн ирц', value: stats.todayAttendance, color: 'text-rose-700', bg: 'bg-rose-50', iconColor: 'text-rose-600', icon: ic('M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z') },
])
